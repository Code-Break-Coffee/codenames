...
export function DeckCard({
  ...
  revealed = false,
  pending = false,
  serverRevealed = false,
  concealerView = false,
  revealWordsOnGameOver = false,
  clickedBy = [],
}) {
  const teamStyles = {
    red: {
  ...
  const defaultStyle = {
    ...
    shine:
      'from-amber-200/0 via-amber-100/40 to-amber-200/0 dark:from-amber-700/0 dark:via-amber-600/20 dark:to-amber-700/0',
  };
...
/>
                ...
                <div className="relative">
                  ...
                  <div className="opacity-0 invisible peer-hover:opacity-100 peer-hover:visible transition-all duration-150 transform scale-95 peer-hover:scale-100 absolute right-0 mt-2 w-max max-w-xs">
                    <div className="bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg p-2 text-xs">
                      {clickedBy.map((n, i) => (
                        <div key={i} className="py-0.5 px-1 truncate">
                          {n}
                        </div>
                      ))}
                    </div>
      ...
      <div
        className={`absolute -inset-1 ${style.bg} rounded-[10px] blur-md opacity-0 group-hover:opacity-30 transition-opacity duration-500`}
      />
      <div
        className={`
      ...
      >
          ...
          {response
            ? createPortal(
                ...
                >
                  ...
                  <div
                    className="relative w-full max-w-2xl max-h-[70vh] overflow-auto bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 border border-gray-200 dark:border-gray-700 rounded-lg shadow-2xl p-4"
        ...
        <div className="absolute inset-0 opacity-0 group-hover:opacity-60 transition-opacity duration-700">
          <div
            className={`absolute inset-0 bg-gradient-to-r ${style.shine} translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000 skew-x-12`}
          />
        </div>
        ...
        {confirmButton && !serverRevealed ? (
          <button
            onClick={(e) => {
              e.stopPropagation();
              if (typeof clickConfirm === 'function') clickConfirm(e);
            }}

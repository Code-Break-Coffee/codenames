...
const TeamPanel = ({ team, score, concealers, revealers, isMyTurn }) => {
  ...
  return (
    ...
    >
      {/* Team Header */}
      <div
        className={`text-center mb-6 p-3 rounded-xl ${colorClasses.bg} ${colorClasses.headerText} shadow-lg ${colorClasses.shadow}`}
      >
        <h2 className="text-2xl font-bold uppercase tracking-wider">{team} Team</h2>
      </div>
      ...
      <div className="text-center mb-8">
        ...
        <div
          className={`inline-flex items-center justify-center w-20 h-20 rounded-full border-4 ${colorClasses.border} ${colorClasses.bg} ${colorClasses.headerText} text-4xl font-extrabold shadow-inner`}
        >
          {score}
        </div>
      </div>
